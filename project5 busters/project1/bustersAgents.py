    ...
    def chooseAction(self, gameState):
        ...
        for distribution in livingGhostPositionDistributions:
            ...
            for key in distribution.keys():
                if distribution[key] >= p_max:
                    p_max = distribution[key]
                    pos_max = key
            ghost_pos.append(pos_max)
        
        dis = {}
        for pos in ghost_pos:
            dis[pos] = self.distancer.getDistance(pacmanPosition, pos)
        
        sort_dis = sorted(dis.items(), key = lambda x: x[1])
        min_pos = sort_dis[0][0]
            
        result = None
        min_dis = float("inf")
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            to_nearest_ghost = self.distancer.getDistance(successorPosition, min_pos)
            if to_nearest_ghost < min_dis:
                min_dis = to_nearest_ghost
                result = action
