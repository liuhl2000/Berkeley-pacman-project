...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            if (self.getQValue(state, action) > max_v):
              max_v = self.getQValue(state, action)
        return max_v
    ...
    def computeActionFromQValues(self, state):
        ...
        if (len(action_list) == 0):
          return best_action
        for action in self.getLegalActions(state):
            if (self.Q_values[(state, action)] > max_v):
              max_v = self.Q_values[(state, action)]
              best_list = [action]
            if (self.Q_values[(state, action)] == max_v):
              best_list.append(action)
        best_action = random.choice(best_list)
        return best_action
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for item in self.featExtractor.getFeatures(state, action):
            result += self.weights[item] * feature[item]
        return result
    ...
    def update(self, state, action, nextState, reward):
        ...
        for item in feature:
            self.weights[item] = self.weights[item] + self.alpha * diff * feature[item]
