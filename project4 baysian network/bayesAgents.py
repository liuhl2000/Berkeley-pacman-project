...
def fillObsCPT(bayesNet, gameState):
    ...
    match_dict = {BOTTOM_LEFT_VAL: bottomLeftPos, BOTTOM_RIGHT_VAL: bottomRightPos, \
        TOP_LEFT_VAL: topLeftPos, TOP_RIGHT_VAL: topRightPos}
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assign in obsFactor.getAllPossibleAssignmentDicts():
                ghost_walls = gameState.getHouseWalls(match_dict[assign[GHOST_HOUSE_VAR]])
                food_walls = gameState.getHouseWalls(match_dict[assign[FOOD_HOUSE_VAR]])
                color = assign[obsVar]
                if obsPos in food_walls:
                    if color == BLUE_OBS_VAL:
                        p = 1 - PROB_FOOD_RED
                    elif color == RED_OBS_VAL:
                        p = PROB_FOOD_RED
                    else:
                        p = 0
                
                elif obsPos in ghost_walls:
                    if color == BLUE_OBS_VAL:
                        p = 1 - PROB_GHOST_RED
                    elif color == RED_OBS_VAL:
                        p = PROB_GHOST_RED
                ...
                obsFactor.setProbability(assign, p)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assign in factor.getAllPossibleAssignmentDicts():
        temp = factor.getProbability(assign)
        if temp > p:
            p = temp
            result = assign
    return result
